// Movie Review Website JavaScript
...
function displayMovies(filteredMovies = movies) {
    ...
    if (filteredMovies.length === 0) {
        container.html('<div class="col-12 text-center"><h4 class="text-muted">No movies found matching your criteria.</h4></div>');
        return;
    }
...
function createMovieCard(movie) {
    const stars = generateStars(movie.rating / 2); // Convert 10-point scale to 5-point
            ...
            <div class="movie-card">
                ...
                <div class="movie-info">
                    ...
                    <div class="movie-meta">
                        <span class="badge bg-warning text-dark me-2">${movie.genre}</span>
                        <span class="text-muted">${movie.year} • ${movie.duration}</span>
                    </div>
                    ...
                    <div class="movie-actions">
                        <button class="btn btn-watch" onclick="showMovieDetails(${movie.id})">
                            <i class="fas fa-play me-1"></i>Watch
                        </button>
                        <button class="btn btn-review" onclick="openReviewModal(${movie.id})">
...
function displayReviews() {
    ...
    reviews.slice(0, 6).forEach(review => { // Show only latest 6 reviews
        ...
        <div class="col-lg-6 mb-4">
            <div class="review-card">
                ...
                <div class="review-meta">
                    <strong>${review.movieTitle}</strong> • ${review.reviewerName}
                </div>
...
function setupEventListeners() {
    ...
    $('#genreFilter, #yearFilter, #ratingFilter').change(handleFilter);
...
function handleFilter() {
    ...
    if (rating) {
        filteredMovies = filteredMovies.filter(movie => movie.rating >= parseFloat(rating) * 2);
    }
...
// Populate movie select in review modal
function populateMovieSelect() {
    const select = $('#movieSelect');
...
// Open review modal
function openReviewModal(movieId = null) {
    if (movieId) {
...
function handleReviewSubmission(e) {
    ...
    const movieId = parseInt($('#movieSelect').val());
...
// Initialize star rating for existing reviews
function initializeStarRatings() {
    $('.rating-stars').each(function() {
        const rating = $(this).data('rating') || 0;
        $(this).html(generateStars(rating));
    });
}
